...
% |NEXTNAME|> returns a file or folder name, incrementing a numbered
% suffix which is appended to the end of the name to ensure that
% returned name is not currently used by any file or folder.
%% Basic Usage
...
S = dir('A_*.txt');
{S.name}.'
S = natsortfiles(S); % alphanumeric order
{S.name}.'